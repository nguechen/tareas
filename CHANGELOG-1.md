...
## v23 — Barra de formato y copiar elementos

- **Barra de formato Markdown** dentro del editor de texto: **B** negrita, *I* cursiva, ~~S~~ tachado, `</>` código, **H** título, **•** lista y **❝** cita. Selecciona el texto y toca el botón. Se pueden combinar (negrita + cursiva) y tocando otra vez se quita.
- **Botón 📋 para copiar** en cada elemento, junto a la lupa. Las tablas y bases de datos se copian **con su estructura**: al pegarlas en Google Sheets o Excel mantienen filas y columnas. Las notas Markdown se copian en crudo, y las listas de pendientes como `- [ ]` / `- [x]`.

## v22 — Tipos de columna como símbolos

- En las bases de datos, el tipo de cada columna se muestra como símbolo en vez de la palabra completa: **Aa** texto · **#** número · **▾** selección · **☑** casilla · **📅** fecha. Al tocarlo se abre la lista con los nombres completos.

## v21 — Arreglo: la lista de tareas vuelve a desplazarse

- Se corrige que la página de tareas no permitía hacer scroll. El contenedor que se añadió al convertir Notas en sección propia (v15) se había quedado sin estilo y rompía la cadena de altura, impidiendo que la lista se desplazara. Arreglo de una línea de CSS; no afecta a datos ni sincronización.
